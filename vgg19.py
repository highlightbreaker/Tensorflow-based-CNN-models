...
class vgg19(Network):
    ...
    def build_network(self, sess, is_training=True):
        with tf.variable_scope('vgg_19', 'vgg_19'):
    ...
    def get_variables_to_restore(self, variables, var_keep_dic):
        ...
        for v in variables:
            # exclude the conv weights that are fc weights in vgg19
            if v.name == 'vgg_19/fc6/weights:0' or v.name == 'vgg_19/fc7/weights:0':
                self._variables_to_fix[v.name] = v
                continue
            # exclude the first conv layer to swap RGB to BGR
            if v.name == 'vgg_19/conv1/conv1_1/weights:0':
                self._variables_to_fix[v.name] = v
                continue
    ...
    def fix_variables(self, sess, pretrained_model):
        print('Fix VGG19 layers..')
        with tf.variable_scope('Fix_VGG19'):
            with tf.device("/gpu:0"):
                # fix the vgg19 issue from conv weights to fc weights
                # fix RGB to BGR
                fc6_conv = tf.get_variable("fc6_conv", [7, 7, 512, 4096], trainable=False)
                fc7_conv = tf.get_variable("fc7_conv", [1, 1, 4096, 4096], trainable=False)
                conv1_rgb = tf.get_variable("conv1_rgb", [3, 3, 3, 64], trainable=False)
                restorer_fc = tf.train.Saver({"vgg_19/fc6/weights": fc6_conv,
                                              "vgg_19/fc7/weights": fc7_conv,
                                              "vgg_19/conv1/conv1_1/weights": conv1_rgb})
                restorer_fc.restore(sess, pretrained_model)

                sess.run(tf.assign(self._variables_to_fix['vgg_19/fc6/weights:0'], tf.reshape(fc6_conv,
                                                                                              self._variables_to_fix['vgg_19/fc6/weights:0'].get_shape())))
                sess.run(tf.assign(self._variables_to_fix['vgg_19/fc7/weights:0'], tf.reshape(fc7_conv,
                                                                                              self._variables_to_fix['vgg_19/fc7/weights:0'].get_shape())))
                sess.run(tf.assign(self._variables_to_fix['vgg_19/conv1/conv1_1/weights:0'],
